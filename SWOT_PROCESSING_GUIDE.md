...
The Automation tab uses the same project database records and reports as the manual tabs. Its date range is explicit and can be copied from the Download tab with `Copy Download Date Range`; the GUI warns when Automation dates differ from Download dates so manual and unattended runs do not accidentally process different temporal windows. Preflight classifies selected UTM tiles as already complete, partial/resumable, needing update, or new for the requested date range. Upload-enabled preflight automatically runs one paginated Earth Engine asset sync before classification and blocks if verification fails, preventing stale inventory data from driving upload or cleanup decisions. The optional `Start automatically after successful preflight` setting begins execution as soon as preflight passes. A run stopped with `Stop After Current Stage` retains its completed stage records and can continue with `Resume Run`; a fresh preflight creates a new current-state plan. Automation writes run logs under `00_logs/automation_runs/<run_id>` and deletes intermediate files only when downstream project records or Earth Engine verification prove the next stage exists.
...
Automation recovery depends on how the run ended. An intentional `Stop After Current Stage` should normally be continued with `Resume Run`, without another preflight. A fully completed queue requires a new preflight for the next date window or tile selection. After a power loss or unexpected shutdown, resume normally unless upload was active. For an interrupted upload, allow Earth Engine tasks to settle and run a fresh upload-enabled preflight so the automatic asset sync verifies successful submissions before the missing assets are replanned.
...
For the current Earth Engine dialog, per-file submission waits for dialog closure rather than repeatedly switching to the Tasks panel. Dialog closure records `SUBMITTED_PENDING_VERIFICATION` and allows the next file to start. Browser-side percentage/progress changes reset the idle timeout, up to `upload.submission_transfer_timeout_minutes`; an open dialog with no progress for `upload.submission_confirmation_timeout_seconds` is treated as stalled, saved as debug evidence, recovered, and retried. Tasks are reconciled at batch boundaries, followed by Earth Engine inventory verification. `ERROR`, `UNKNOWN_AFTER_CLICK`, and unverified pending rows preserve the local mosaic and remain retryable after EE sync confirms that the destination asset is absent. Automation continues to later tiles and cleans only mosaics with verified downstream assets.
